...
conn = ms.connect(host="localhost", port=3306, user="root", passwd="Edchaz168", database="dbms_proj")
  ...
  if request.method == 'POST':
    ...
    if result!=[]:
      ...
      if result_fixed!=[] and result_daily==[]:
        ...
        return render_template("dashboard.html",total_expense=-total_expense,fixed_expense=fixed_expense,daily_expense=0,total_income=total_income,result_loan=result_loan,result_income=result_income,result_fixed=result_fixed,uname=uname,fname=fname,lname=lname,email=email,pno=pno,budget=budget,savings=savings)
        ...
        conn.commit()
        budget=budget-total_expense
        return render_template("dashboard.html",total_expense=-total_expense,fixed_expense=0,daily_expense=daily_expense,total_income=total_income,result_loan=result_loan,result_income=result_income,result_today=result_today,uname=uname,fname=fname,lname=lname,email=email,pno=pno,budget=budget,savings=savings)
      elif result_fixed!=[] and result_daily!=[]:
        mc.execute("select sum(amount) from fixed_expenses where uname=%s",(uname,))
        l=mc.fetchall()
        print(l)
        ...
        return render_template("dashboard.html",daily_expense=0,fixed_expense=0,uname=uname,total_income=0,fname=fname,lname=lname,email=email,pno=pno,budget=budget,savings=savings)
    else:
...
def user_dashboard_page():
      ...
      if result_fixed!=[] and result_daily==[]:
        ...
        conn.commit()
        budget=budget-total_expense
        return render_template("dashboard.html",total_expense=-total_expense,daily_expense=0,fixed_expense=fixed_expense,total_income=total_income,result_loan=result_loan,result_income=result_income,result_fixed=result_fixed,uname=uname,fname=fname,lname=lname,email=email,pno=pno,budget=budget,savings=savings)
      elif result_fixed==[] and result_daily!=[]:
        mc.execute("select sum(amount) from daily_expenses where uname=%s",(uname,))
        l=mc.fetchall()
        print(l)
        ...
        return render_template("dashboard.html",daily_expense=0,fixed_expense=0,total_income=0,uname=uname,fname=fname,lname=lname,email=email,pno=pno,budget=budget,savings=savings)
